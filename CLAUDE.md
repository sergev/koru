...
`DEBUG_ATOMIC_SLEEP` plus lockdep in the dev kernel is what catches a slip. The submitter
`Mutex` is likewise dropped around the `CondVar` wait, or one waiter blocks every submitter.

Wait semantics: `min_complete` decides whether `ENTER` waits at all, `timeout_ns` only caps

...
int main(void)
{
    ...
    enter_init(&e, sq, 1, cq, 16);
    e.reserved[1] = 1;
    check_errno(ioctl(fd, KORU_IOC_ENTER, &e), EINVAL, "non-zero ENTER reserved returns EINVAL");

    enter_init(&e, sq, SQ_ENTRIES + 1, cq, 16);
    check_errno(ioctl(fd, KORU_IOC_ENTER, &e), EINVAL, "to_submit over sq_entries returns EINVAL");

    enter_init(&e, sq, 1, cq, 2);
    e.min_complete = 3;
    check_errno(ioctl(fd, KORU_IOC_ENTER, &e), EINVAL, "min_complete over cq_space returns EINVAL");

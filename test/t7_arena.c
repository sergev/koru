...
int main(void)
{
    ...
    check_errno(setup_ring(fd, 32, 64, 4097, 4), EINVAL,
                "SETUP with slot_size 4097 returns EINVAL");
    check(setup_ring(fd, 32, 64, SLOT_SIZE, SLOT_COUNT) == 0,
          "SETUP with a page multiple succeeds");
    ...
    check(WIFEXITED(status) && WEXITSTATUS(status) == 0, "a forked child's maps lack the region");
